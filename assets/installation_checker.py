    ...
    try:
        ...
        if current_drive.upper() != system_drive.upper():
            raise InstallationError(
                f"Installation Error: The current working directory is on drive {current_drive}, but the default system drive is {system_drive}. Please move Shiromiya RVC Fork to the {system_drive} drive."
            )
    except:
        pass
    else:
