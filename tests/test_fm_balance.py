...
def test_the_weight_falls_as_feature_matching_outgrows_reconstruction(rule):
    ...
    # configured ``chouwagan_fm_weight`` still mean something.
...
def test_the_config_ships_the_knobs():
    ...
    assert train["chouwagan_fm_balance_target"] == 0.33
    assert train["chouwagan_fm_balance_min"] == 0.1
    ...
    assert train["chouwagan_r1_max_scale"] == 10000.0
