...
def get_cosine_fade(length, device):
    # Raised cosine (half-Hanning): smoother than a linear fade, protects spectral integrity.
...
def find_cuts(mask, waveform, sr):
    # Finds rough cut points from the mask, filters segments < 100ms, then
    # snaps each cut to the nearest zero crossing to avoid clicks/pops.
    ...
    # 1 marks the start of a silence run, -1 the end.
    ...
    # Shouldn't invert since min > 2*buffer (100ms > 10ms), but guard anyway.
    ...
    # Pure silence or DC offset: no crossings to snap to, fall back to rough cuts.
    ...
    # Beyond the search window, the nearest zero-crossing isn't safe to use.
...
def SmartCutter(waveform, mask, sr=48000):
    ...
    if ENABLE_BRIDGING:
        ...
        mask = torch.nn.functional.max_pool1d(mask, bridge_frames, 1, bridge_frames//2)  # dilate
        mask = -torch.nn.functional.max_pool1d(-mask, bridge_frames, 1, bridge_frames//2)  # erode
...
def process_grid_aligned(model, transform, waveform, sr, hop_length, device, static_input_buffer):
    # Weighted Overlap-Add (WOLA) inference over overlapping chunks.
    ...
    # Hann window so the center of each chunk's prediction counts more than the edges.
...
def _run_inference(model, mel_transform, wav_chunk, device, input_buffer):
    ...
    # Deltas add temporal context as a second input channel.
    ...
    # Collapse the 2D (freq, time) output to 1D (time) per MASK_MODE.
    if MASK_MODE == "Soft":
...
def processing():
    if FORCE_CPU:
        ...
        torch.backends.cudnn.benchmark = False  # disabled for run-to-run consistency
    ...
    for f_path in files:
        try:
            ...
            # Downmix stereo by picking the channel with the lowest DC offset.
            if wav.shape[0] > 1:
            ...
            if sr not in loaded_models:
                ...
                # Only one model kept resident at a time; free it before loading another.
                ...
                if sr in [48000, 40000]:
                    ...
                    N_FFT = 1024  # 32kHz model variant
                    N_MELS = 128
                ...
                # Static input buffer: [Batch, Channels(Mel+Delta), Mel_Bins, Frames_per_chunk]
