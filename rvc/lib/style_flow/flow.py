...
def flow_loss(model, batch: dict, *, unit_dropout=0.0, descriptor_dropout=0.0, t=None, noise=None, t_mean=0.0, t_std=1.0):
    ...
    Returns ``(loss, per_sample_loss, t, estimate, drop_desc)``, ``estimate``
    being the target the prediction points to, ``x_t + (1 - t) v``, and
    ``drop_desc`` which samples saw no descriptors."""
    ...
    v = model(
        ...
        drop_units=drop_units, drop_desc=drop_desc, source=batch.get("source"), loudness=batch.get("loudness"),
    )
    m = mask[:, None].float()
    err = ((v.float() - (x1 - x0)) ** 2 * m).sum(dim=(1, 2))
    per_sample = err / (m.sum(dim=(1, 2)) * x1.shape[1]).clamp(min=1.0)
    estimate = (x_t + (1.0 - tt) * v.float()) * m
    return per_sample.mean(), per_sample, t, estimate, drop_desc
...
def sample(
    ...
    *,
    ...
    ``cfg_drop="all"``, and stops at ``t >= cfg_until``: that close to the
    data it only sharpens fine-detail differences, which come out as wiggles.
    ``convert`` is a converter's source input, ``loudness`` the level input
    of a model that takes one."""
    B, T = units.shape
    ...
    if guided:
        ...
        convert_cfg = None if convert is None else torch.cat([convert, convert])
        loudness_cfg = None if loudness is None else torch.cat([loudness, loudness])

    ts = torch.linspace(t0, 1.0, steps + 1, device=units.device)
    guide = [guided and t < cfg_until for t in ts.tolist()]
    for i in range(steps):
        t = ts[i].expand(B)
        if guide[i]:
            v = model(
                torch.cat([x, x]), torch.cat([t, t]), *cond, drop_units=drop_units, drop_desc=drop_desc,
                source=convert_cfg, loudness=loudness_cfg,
            ).float()
            v_cond, v_uncond = v.chunk(2)
            v = v_uncond + cfg_scale * (v_cond - v_uncond)
        else:
            v = model(x, t, units, coarse, desc, desc_mask, mask, source=convert, loudness=loudness).float()
        x = x + (ts[i + 1] - ts[i]) * v
    return x * mask[:, None]
