...
whatever consumes it.  ``tools/pretrain/clean_pretrain.py`` is what converts a
...
So: stage 2 -> stage 3 -> ``tools/pretrain/clean_pretrain.py``.

WHEN TO MOVE ON
---------------
When ``loss_kl`` has flattened and ``diag/prior_gap_mel_l1`` has stopped
falling, run ``tools/pretrain/pretrain_stage3_endtoend.py`` on the same folder.

USAGE
-----
    python tools/pretrain/pretrain_stage2_encoders.py --model vctk-vocoder \\
        --total-epochs 70
...
# ``python tools/pretrain/<script>.py`` puts ``tools/pretrain/`` on ``sys.path``, not the repo
...
from tools.pretrain._staged_pretrain import (
    StageError,
...
def main() -> int:
    ...
    if not checkpoints and not args.allow_no_checkpoint:
        raise StageError(
            ...
            f"trained. Run tools/pretrain/pretrain_stage1_vocoder.py on this folder "
