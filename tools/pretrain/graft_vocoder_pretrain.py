...
WHAT THIS DOES
--------------
``tools/pretrain/pretrain_stage0_vocoder.py`` trains a decoder whose input is a mel
...
``tools/pretrain/pretrain_stage0_vocoder.py``.

USAGE
-----
    # defaults: newest stage0_*.pth in the stage-0 folder, EMA weights
    python tools/pretrain/graft_vocoder_pretrain.py --model vctk-vocoder \\
...
Then start stage 1 from it, in the target model's folder:
    ...
    python tools/pretrain/pretrain_stage1_vocoder.py --model my-voice \\
...
from tools.pretrain._staged_pretrain import (  # noqa: E402
    StageError,
    model_dir,
    resolve_vocoder,
)
...
def newest(directory: Path, kind: str) -> Path:
    ...
    if best is None:
        raise StageError(
            ...
            f"tools/pretrain/pretrain_stage0_vocoder.py there first."
        )
    return best[1]
...
def main() -> int:
    ...
    print(
        f"\nNext: python tools/pretrain/pretrain_stage1_vocoder.py --model "
